	...
	const saveCredentials = () => {
		...
		if (editIndex !== null) {
			updatedStreams = streams.map((stream, i) =>
				i === editIndex
					? { ...formData, datetime_added: stream.datetime_added, settings: stream.settings || defaultSettings }
					: stream
			);
		} else {
			updatedStreams = [
	...
	return (
		<>
			{streams.map((stream, i) => (
				<div key={i} className="relative block w-full rounded-lg border-2 border-primary-700 p-6 text-center group">
					<button
			...
			>
				...
				<span className="mt-2 block text-sm font-semibold text-primary-800 group-hover:text-primary-700 duration-300">
					Add IPTV stream
				</span>
			</button>
				...
				<div className="fixed inset-0 flex w-screen items-center justify-center p-4 h-screen bg-dark/75">
					<DialogPanel className="max-w-lg space-y-4 bg-dark-300 rounded-xl p-6 text-secondary-600">
						...
						<div className="grid grid-cols-2 gap-4">
							<button
								className="py-1.5 rounded-xl bg-dark-400 hover:bg-dark-500 duration-300"
								onClick={() => setIsOpen(false)}
							>
								Cancel
							</button>
							<button
								className="py-1.5 rounded-xl bg-secondary-400 hover:bg-secondary-500 hover:text-secondary-400 duration-300"
